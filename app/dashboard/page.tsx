...
import {
  IconFileText, IconSend, IconCircleCheck, IconClock, IconAlertCircle, IconPlus, IconArrowRight,
    ...
    <div className="anim-fade-in">
      ...
      <div className="stat-tabs anim-stagger">
        <Link href="/documents" className="stab">
          <div className="stab-header">
            <div className="stab-icon ic-blue"><IconFileText size={14} /></div>
        ...
        <Link href="/documents?status=DRAFT" className="stab">
          <div className="stab-header">
            <div className="stab-icon ic-amber"><IconInbox size={14} /></div>
        ...
        <Link href="/documents?status=APPROVED" className="stab">
          <div className="stab-header">
            <div className="stab-icon ic-green"><IconCircleCheck size={14} /></div>
      ...
      <div className="grid2 anim-stagger">
        ...
        <div className="card">
          <div className="ch">
            <div className="ch-l">
              {isAdmin ? <IconLayoutGrid size={14} /> : isValidator ? <IconClock size={14} /> : isRegistrar ? <IconInbox size={14} /> : <IconFileText size={14} />}
              ...
              <>
                ...
                <div style={{ marginTop: 10, paddingTop: 10, borderTop: "1px solid var(--bg-hover)" }}>
                  <Link href="/admin/workflows" className="ag-link">
                    <IconSettings size={11} style={{ verticalAlign: -1, marginRight: 3 }} />
              ...
              <>
                ...
                {overdueApprovals.some(a => {
                  ...
                  <div style={{ marginTop: 8 }}>
                    <div style={{ fontSize: 10.5, fontWeight: 600, color: "var(--danger)", marginBottom: 6, display: "flex", alignItems: "center", gap: 4 }}>
                      <IconClock size={12} /> Просроченные документы
                    </div>
                    {overdueApprovals.filter(a => {
                      ...
                      <Link key={a.id} href={`/documents/${a.document.id}`} className="doc-item">
                        <div className="doc-ico ic-amber"><IconClock size={14} /></div>
                        <div className="doc-info">
                ...
                {stats.pendingApprovals === 0 && (
                  <div className="empty-state" style={{ padding: "20px 10px" }}>
                    <IconCircleCheck size={24} style={{ color: "var(--success)", marginBottom: 6, display: "block", margin: "0 auto 6px" }} />
                ...
                {myInProgress.length > 0 ? myInProgress.map(doc => (
                  <Link key={doc.id} href={`/documents/${doc.id}`} className="doc-item">
                    <div className={`doc-ico ${typeIconMap[doc.type] || "ic-blue"}`}>
                       {doc.type === "REPORT" ? <IconBook size={14} /> :
                        doc.type === "MEMO" ? <IconEdit size={14} /> :
                        doc.type === "CONTRACT" ? <IconSignature size={14} /> :
            ...
            {isRegistrar && (
              <div className="admin-grid">
                <Link href="/incoming/register" className="ag-item">
                  <div className="ag-icon ic-blue"><IconPlus size={16} /></div>
                  <div className="ag-lbl">Регистрация</div>
                </Link>
                <Link href="/incoming" className="ag-item">
                  <div className="ag-icon ic-purple"><IconInbox size={16} /></div>
                  <div className="ag-lbl">Входящие</div>
                </Link>
                <Link href="/archive" className="ag-item">
                  <div className="ag-icon ic-amber"><IconArchive size={16} /></div>
      ...
      {activity.length > 0 && (
        <div className="card anim-stagger">
          <div className="ch">
            <div className="ch-l"><IconActivity size={14} />Последние действия</div>
